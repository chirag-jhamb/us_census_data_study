...
# March 20, 2019
#
# acs_county_page_plots_2017_v02.R
#
##############################################################################
##############################################################################
...
# load the data
acs_cnt_2012_2016 <- read.csv(paste0(data_dir,"20190303_cnty_acs_2013_2017_absolute_values.csv"),stringsAsFactors = F)
...
#save(acs_cnt_2012_2016_subset,file="census_data_2017.RData")
...
built_cols <- c("2000 to 2009","1990 to 1999" ,"1980 to 1989", "1970 to 1979", "2010 to 2016", "before 1949" , "1950 to 1969")
...
df_counties_1 <- acs_cnt_2012_2016_subset_built_year_melt %>% 
...
#df <- acs_cnt_2012_2016_subset_built_year_melt
#statefips <- "00"
#countyfips <- "000"
...
df_counties_2 <- acs_cnt_2012_2016_subset_structure_type_melt %>% 
  group_by(STATE_FIPS,county_code,county_name,variable) %>% 
  summarise(value_metro=sum(value)) %>% mutate(value_prop=round(value_metro/sum(value_metro, na.rm = T)*100,2)) %>% 
  as.data.frame()
...
df_counties_3 <- acs_cnt_2012_2016_subset_owner_occupied_melt %>% 
  group_by(STATE_FIPS,county_code,county_name,variable) %>% 
  summarise(value_metro=sum(value)) %>% mutate(value_prop=round(value_metro/sum(value_metro, na.rm = T)*100,2)) %>% 
  as.data.frame()
...
df_counties_4 <- acs_cnt_2012_2016_subset_household_income_melt %>% 
  group_by(STATE_FIPS,county_code,county_name,variable) %>% 
  summarise(value_metro=sum(value)) %>% mutate(value_prop=round(value_metro/sum(value_metro, na.rm = T)*100,2)) %>% 
  as.data.frame()
...
df_counties_5 <- acs_cnt_2012_2016_subset_race_melt %>% 
  group_by(STATE_FIPS,county_code,county_name,variable) %>% 
  summarise(value_metro=sum(value)) %>% mutate(value_prop=round(value_metro/sum(value_metro, na.rm = T)*100,2)) %>% 
  as.data.frame()
...
df_counties_6 <- acs_cnt_2012_2016_subset_household_type_melt %>% 
  group_by(STATE_FIPS,county_code,county_name,variable) %>% 
  summarise(value_metro=sum(value)) %>% mutate(value_prop=round(value_metro/sum(value_metro, na.rm = T)*100,2)) %>% 
  as.data.frame()
