...
# load the data
acs_zcta_2012_2016 <- read.csv(paste0(data_dir,"20190226_acs_zcta_2012_2016.csv"),stringsAsFactors = F)
...
plot_zcta_level_quantile_values <- function(df,colname,area_type){
    ...
    p <- ggplot(df.int) +
      ...
      #geom_sf(data = national_highway_interstate.int, fill=NA, colour = "green", size=0.5)+
      geom_sf(data = cborders1960c, fill=NA, colour = "grey", size=0.5)+
      coord_sf() +
      theme_map() +
...
col_vec <- c("new_existing_constructions_ratio")
...
plot_zcta_level_zero_NA_values <- function(df,colname,area_type,missing){
  ...
  #national_highway_interstate.int <- st_intersection(national_highway_interstate,cborders1960c)
  if(missing=="zero"){
  data <-  subset(df.int, brks==0)
  my.cols[2] <- "#000000"
  }else{
    data <-  subset(df.int, brks==-1)
  }
  # # draw the plot with legend at the bottom
  p <- ggplot(data) +
    geom_sf(aes(fill=brks),colour="grey", size=0.3)+
  ...
  data_sub <- data %>% dplyr::select("ZCTA","new_constructions","existing_constructions","new_existing_constructions_ratio",
                                         "share_single_family_new_constructions", "total_single_family_households_since_2000", "total_single_family_households",
                                         "STATE", "COUNTY" ,"CENSUSAREA","zcta_area","intersected_area","percent_intersected_area","NAME","LSAD","brks")

  st_geometry(data_sub) <- NULL


  if (colname_str=='new_constructions'){
    save_plot <- paste0(out_dir_ch01,"p1.m1.",area_type,"_",dateo,"_acs_zcta_quantiles_of_",colname_str,"_zctas_",missing,".jpg")
    write.csv(data_sub,paste0(out_dir_ch01,"p1.m1.",area_type,"_",dateo,"_acs_zcta_of_",colname_str,"_zctas_",missing,".csv"),row.names = F)
  } else if (colname_str=='new_existing_constructions_ratio'){
    save_plot <- paste0(out_dir_ch01,"p1.m2.",area_type,"_",dateo,"_acs_zcta_quantiles_of_",colname_str,"_zctas_",missing,".jpg")
    write.csv(data_sub,paste0(out_dir_ch01,"p1.m2.",area_type,"_",dateo,"_acs_zcta_of_",colname_str,"_zctas_",missing,".csv"),row.names = F)
  } else if (colname_str=='share_single_family_new_constructions'){
    save_plot <- paste0(out_dir_ch02,"p2.m1.",area_type,"_",dateo,"_acs_zcta_quantiles_of_",colname_str,"_zctas_",missing,".jpg")
    write.csv(data_sub,paste0(out_dir_ch02,"p2.m1.",area_type,"_",dateo,"_acs_zcta_of_",colname_str,"_zctas_",missing,".csv"),row.names = F)
  }
  # else if (colname_str=='Median.value..dollars'){
  #   save_plot <- paste0(out_dir_ch03,"p3.m1.",area_type,"_",dateo,"_acs_zcta_quantiles_of_",colname_str,".jpg")
  # }  else {
...
# call the funtion to create plot for each variable
for (col in col_vec){
  plot_zcta_level_zero_NA_values(zcta_2016,quo(!!sym(col)),"00000","zero")
  plot_zcta_level_zero_NA_values(zcta_2016,quo(!!sym(col)),"11111","zero")
  plot_zcta_level_zero_NA_values(zcta_2016,quo(!!sym(col)),"00000","NAs")
  plot_zcta_level_zero_NA_values(zcta_2016,quo(!!sym(col)),"11111","NAs")
}
...
#   
#   if(statefips!="00"){
#     df <- df %>% filter(STATE==statefips & COUNTY==countyfips)
#   }
#   
#   #df <- df %>% filter(area_type!="Exurban")
#   
#   # # draw the plot with legend at the bottom
#   p <- ggplot(df) +
...
#   
#   if (colname_str=='new_constructions'){
#     save_plot <- paste0(out_dir,"c1.m1.",statefips,countyfips,"_",dateo,"_acs_zcta_quantiles_of_",colname_str,".jpg")
#   } else if (colname_str=='total_single_family_households_since_2000'){
#     save_plot <- paste0(out_dir,"c2.m1.",statefips,countyfips,"_",dateo,"_acs_zcta_quantiles_of_",colname_str,".jpg")
#   } else if (colname_str=='total_single_family_households'){
#     save_plot <- paste0(out_dir,"c2.m1.",statefips,countyfips,"_",dateo,"_acs_zcta_quantiles_of_",colname_str,".jpg")
#   } else if (colname_str=='Median.value..dollars'){
#     save_plot <- paste0(out_dir,"c3.m1.",statefips,countyfips,"_",dateo,"_acs_zcta_quantiles_of_",colname_str,".jpg")
#   }  else {
#     save_plot <- paste0(out_dir,"c3.m1.",statefips,countyfips,"_",dateo,"_acs_zcta_quantiles_of_",colname_str,".jpg")
#   }
#   
...
# 
# col_vec <- c("new_constructions","total_single_family_households_since_2000")
# 
...
# # call the funtion to create plot for each variable
# # for (col in col_vec){
# #   plot_zcta_level_quantile_values(zcta_2016,quo(!!sym(col)),"00","000")
# # }
# 
# state_fips <- c("11","00")
# 
