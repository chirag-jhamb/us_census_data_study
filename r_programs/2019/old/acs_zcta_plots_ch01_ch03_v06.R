...
plot_zcta_level_quantile_values <- function(df,colname,area_type){
  ...
  p <- ggplot(df.int) +
    ...
    geom_sf(data = metro_dc, fill=NA, colour = "blue", size=0.5)+
    geom_sf(data = national_highway_interstate.int, fill=NA, colour = "#737373", size=0.5)+
...
col_vec <- c("new_existing_constructions_ratio","share_single_family_new_constructions")
