...
out_dir_county <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2019/ch01/")
...
reformat_subset_data <- function(df) {
  ...
  ## for year 2011-2014
  #data %>% select(matches('^1, detached.*attached$|attached.5$')) %>% head(2)
  #data %>% select(matches('^2 to * 4$|4.5$')) %>% head(2)
  #data %>% select(setdiff(starts_with('1, detached'),matches('attached$|attached.5$'))) %>% head(2)
  #data %>% select(setdiff(starts_with('2 to'),matches('to 4$|4.5$'))) %>% head(2)
  ...
  data <- data %>% mutate('total_single_family_households_since_2000'=select(.,matches('^1, detached.*attached$|attached.5$')) %>%
                            apply(1, sum, na.rm=TRUE))

  data <- data %>% mutate('total_single_family_households_before_2000'=select(.,setdiff(starts_with('1, detached'),
                                                                    matches('attached$|attached.5$'))) %>%  apply(1, sum, na.rm=TRUE))

  data <- data %>% mutate('total_2_to_4_family_households_since_2000'=select(.,matches('^2 to * 4$|4.5$')) %>%
                            apply(1, sum, na.rm=TRUE))

  data <- data %>% mutate('total_2_to_4_family_households_before_2000'=select(.,setdiff(starts_with('2 to'),
                                                                   matches('to 4$|4.5$'))) %>% apply(1, sum, na.rm=TRUE))

  data <- data %>% mutate('total_5_to_19_family_households_since_2000'=select(.,matches('^5 to * 19$|19.5$')) %>%
                            apply(1, sum, na.rm=TRUE))

  data <- data %>% mutate('total_5_to_19_family_households_before_2000'=select(.,setdiff(starts_with('5 to'),
                                                                                        matches('to 19$|19.5$'))) %>% apply(1, sum, na.rm=TRUE))
  data <- data %>% mutate('total_20_to_49_family_households_since_2000'=select(.,matches('^20 to * 49$|49.5$')) %>%
                            apply(1, sum, na.rm=TRUE))


  data <- data %>% mutate('total_20_to_49_family_households_before_2000'=select(.,setdiff(starts_with('20 to'),
                                                                                        matches('to 49$|49.5$'))) %>% apply(1, sum, na.rm=TRUE))

  data <- data %>% mutate('total_50_or_more_family_households_since_2000'=select(.,matches('^50 or * more$|more.5$')) %>%
                            apply(1, sum, na.rm=TRUE))


  data <- data %>% mutate('total_50_or_more_family_households_before_2000'=select(.,setdiff(starts_with('50 or'),
                                                                                          matches('or more$|more.5$'))) %>% apply(1, sum, na.rm=TRUE))
  ...
  data <- data %>% mutate('total_2_to_4_family_households_before_2000'=select(.,setdiff(starts_with('2 to'),
                                                      matches('to 4$|4.1$|4.6$|4.7$'))) %>% apply(1, sum, na.rm=TRUE))
  ...
  data <- data %>% mutate('total_20_to_49_family_households_before_2000'=select(.,setdiff(starts_with('20 to'),
                                                                matches('to 49$|49.1$|49.6$|49.7$'))) %>% apply(1, sum, na.rm=TRUE))
  ...
  data <- data %>% mutate('total_50_or_more_family_households_before_2000'=select(.,setdiff(starts_with('50 or'),
                                                                  matches('or more$|more.1$|more.6$|more.7$'))) %>% apply(1, sum, na.rm=TRUE))
  ...
  data_subset <- data %>% select(STATE_FIPS, county_code, year, Total_population, total_housing_units, Median_Value,
                                 ...
                                 total_50_or_more_family_households_since_2000,total_50_or_more_family_households_before_2000,`Owner occupied:`,
                                 `Renter occupied:`)
...
acs_cnt_1950_2016_year_area_wise_housing <- acs_cnt_1950_2016 %>%
                                                group_by(year, area_type) %>%
                                                summarise(housing_units = sum(total_housing_units, na.rm = T))



# get area wise units before and after 2000
acs_cnt_1950_2016_area_wise_housing <- acs_cnt_1950_2016_year_area_wise_housing %>%
                                                                    group_by(area_type) %>%
                                                                       summarise(housing_before_2000=sum(housing_units[year<2000]),
                                                                                 housing_since_2000=sum(housing_units[year>=2000])) %>%
                                                                                     as.data.frame()

# melt the dataframe
acs_cnt_1950_2016_area_wise_housing_melt <- melt(acs_cnt_1950_2016_area_wise_housing, id.var="area_type")


# plot the graph
p <- ggplot(acs_cnt_1950_2016_area_wise_housing_melt, aes(x = area_type, y = value, fill = variable)) +
...
# filter the data for housing units built post 2000
acs_cnt_1950_2016_after_2000 <- acs_cnt_1950_2016 %>% filter(year>2000) %>% as.data.frame()

# get total housing units grouped by area and county
acs_cnt_1950_2016_after_2000_area_county_wise <- acs_cnt_1950_2016_after_2000 %>%
                                                          group_by(area_type,county_name) %>%
                                                                summarise(housing_units = sum(total_housing_units, na.rm = T)) %>%
                                                                    as.data.frame()

# plot the graph
p <- ggplot(acs_cnt_1950_2016_after_2000_area_county_wise, aes(x = area_type, y = housing_units, fill = county_name)) +
  geom_bar(stat = "identity")+
...
# plot the graph
p1 <- ggplot(data = subset(acs_cnt_2000_2016_comp_abs, year %in% c("2000")), aes(x = median_housing_value, y = housing_units_abs_change_from_2000_prop)) +
  #geom_point(aes(color = factor(damage_eyeball,levels=c("irreparable","extensive","minimal","none")),
  #                fill = factor(damage_eyeball,levels=c("irreparable","extensive","minimal","none"))),size=7) +
  #geom_point(aes(color = factor(STATE)),shape = 16, size = 3)+
  geom_point(shape = 16, size = 5)+geom_text(aes(label=county_name),hjust=-0.1, vjust=0.15)+
...
###################### P2.62 Plot Bi directional Bar chart for new housing units and proportion of single family by county ###################################################
...
acs_cnt_1950_2016_area_wise_owner_renter_occupancy <- acs_cnt_1950_2016 %>%
                                                        group_by(area_type) %>%
                                                          summarise(total_owner_occupied = sum(`Owner occupied:`, na.rm = T),
                                                                    total_renter_occupied=sum(`Renter occupied:`, na.rm = T)) %>%
                                                                            as.data.frame()
...
p <- ggplot(acs_cnt_1950_2016_area_wise_owner_renter_occupancy_melt, aes(x = area_type, y = value, fill = variable)) +
  geom_bar(stat = "identity")+
  scale_y_continuous(labels = comma,  limits = c(0, 7000000), breaks = c(seq(0,7000000,1500000)))+
  #scale_x_continuous(limits= c(1950, 2016), breaks = c(seq(1950,2016,10))) +
  #scale_colour_manual(values = c("orange","green"))+
  labs(x = "area type", y = "num of housing units", colour = "Parameter")+
  scale_shape_manual(values = c(16, 21)) +
...
col_vec <- c("median_housing_value","median_household_income")
...
###################### P3.G2.2 Plot Housing Units Percent change from 200 to 2016 vs Median Housing Value 2000 ###################################################
...
## 1 ##
# acs_dc_data <- acs_cnt_1950_2016 %>% filter(statefips=="11" & countyfips=="001")
# 
# max_val <- round(max(acs_dc_data$total_housing_units),-4)
# brk <- round(max_val/5,-4)
# 
#  p1 <- ggplot(acs_dc_data,aes(x = year, y = total_housing_units)) +
#   geom_bar(stat = "identity")+ #scale_fill_manual(values=my_scale)+
#   #geom_text(aes(label=colname), hjust=-0.1, color="black", size=3.5)+
#   scale_y_continuous(limits= c(0,max_val),breaks = c(seq(0,max_val,brk))) +
#   theme(panel.grid.major = element_blank(),
#         panel.grid.minor = element_blank(),
#         panel.background = element_blank(),
#         #panel.grid.major.y = element_line(color="gray"),
...
# 
# 
#  ggsave(paste0(out_dir_county,"c1_",dateo,"_acs_cnt_2016_total_housing_year_wise.jpg"),
#         plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))
#  
#  ## 3 ##
#  
#  max_val <- round(max(acs_dc_data$`Owner occupied:`,na.rm = T))
#  brk <- round(max_val/4,-4)
#  
#  ggplot(acs_dc_data,aes(x = year, y = `Owner occupied:`)) +
#    geom_bar(stat = "identity")+ #scale_fill_manual(values=my_scale)+
#    #geom_text(aes(label=colname), hjust=-0.1, color="black", size=3.5)+
#    scale_y_continuous(limits= c(0,max_val),breaks = c(seq(0,max_val,brk))) +
#    theme(panel.grid.major = element_blank(),
...
#          panel.grid = element_blank(), panel.border = element_blank())
#  
#  ggsave(paste0(out_dir_county,"c1_",dateo,"_acs_cnt_2016_owner_housing_year_wise.jpg"),
#         plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))
#  
#  ## 4 ##
#  
#  max_val <- round(max(acs_dc_data$median_household_income,na.rm = T))
#  brk <- round(max_val/4,-4)
#  
#  ggplot(acs_dc_data,aes(x = year, y = median_household_income)) +
#    geom_bar(stat = "identity")+ #scale_fill_manual(values=my_scale)+
#    #geom_text(aes(label=colname), hjust=-0.1, color="black", size=3.5)+
#    scale_y_continuous(limits= c(0,max_val),breaks = c(seq(0,max_val,brk))) +
#    theme(panel.grid.major = element_blank(),
...
#  
# 
# ggsave(paste0(out_dir_county,"c1_",dateo,"_acs_cnt_2016_household_income_year_wise.jpg"),
#         plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))