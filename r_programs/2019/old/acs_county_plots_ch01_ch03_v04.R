...
p1 <- ggplot(df, aes(x = area_type, y = total_housing_units, fill = group)) +
  ...
  #scale_y_continuous(labels = scales::comma, breaks = trans_breaks(identity, identity, n = 5), expand = c(0, 0))+
  ...
  #geom_text(aes(label=unit_type), vjust=-0.1, color="black", size=5)+
...
#b1 <- bracketsGrob(150000, 149000, 150000,  148000, lwd=2, col="red")
...
p1 <- ggplot(df, aes(x = area_type, y = total_housing_units, fill =  group)) +
  ...
  #geom_segment(aes(x = 2.4, y = 190080, xend = 2.55 , yend = 190080), arrow = arrow(length = unit(0.3, "cm")))+
  #geom_segment(aes(x = 2.4, y = 190080, xend = 2.55 , yend = 187000), arrow = arrow(length = unit(0.3, "cm")))+
...
#point <- format_format(big.mark = ",", decimal.mark = ".", scientific = FALSE)
p1
#grid.brackets(475, 35, 475, 31, lwd=2, col="#525252")

#dev.copy(png,"mypng.png",height=1000,width=1000,res=200)
#dev.copy(jpeg,"myjpg.jpg",height=1100,width=1600,res=300) 
#dev.off()
  # make the barplot horizontal
#p1 <- p + coord_flip() #+ scale_y_continuous(labels = point,expand = expand_scale(mult = c(0, .1)))
...
p1 <- ggplot(data = acs_cnt_2000_2016_comp_pct_2000 , aes(x = median_housing_value, y = housing_units_pct_change_from_2000)) +
  ...
  #labs(x = "median home value in 2000, $2016", y="percentage change, number of housing units, 2000 to 2016")+
...
# # plot the graph with Loudoun county removed
# p1 <- ggplot(data = subset(acs_cnt_2000_2016_comp_pct, year %in% c("2000") & county_name!="Loudoun County"), aes(x = median_housing_value, y = housing_units_pct_change_from_2000)) +
...
#   geom_text(aes(label=county_name),hjust=-0.1, vjust=0.15, size=5)+
...
#   geom_smooth(method='lm', se=FALSE) + # 45 degree line
#   theme(panel.grid.major = element_blank(),
#         panel.grid.minor = element_blank(),
#         panel.background = element_blank(),
#         panel.grid.major.y = element_line(color="gray"),
...
# 
# # save the graph
# ggsave(paste0(out_dir_ch01,"p1.g3.1_",dateo,"_acs_cnt_comp_2000_2016_scatter_house_median_value_percent_change_housing_units_without_Loudoun.jpg"),
#        plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))
...
write.csv(acs_cnt_2016_new_ext_housing_melt_rel_freq,
          paste0(out_dir_ch02,"p2.g1_",dateo,"_acs_cnt_1950_2016_housing_units_by_unit_type_and_count.csv"),row.names = F)
...
write.csv(acs_cnt_1950_2016_pop_housing,
          paste0(out_dir_intro,"i1.g1_",dateo,"_acs_cnt_1950_2016_population_and_housing_growth.csv"),row.names = F)
