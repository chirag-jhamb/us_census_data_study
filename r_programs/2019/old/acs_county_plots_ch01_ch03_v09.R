...
# March 26, 2019
#
# acs_county_plots_ch01_ch03_v09.R
#
##############################################################################
##############################################################################
...
# 
# acs_cnt_2017_comp_abs <- acs_cnt_2000_2017_comp_abs %>% filter(year=="2017") %>% select(county_name_state_code,area_type,housing_units_abs_change_from_2000)
# 
# # melt the dataframe
...
annotate(geom="text", x=1, y=fairfax_fall_church_arrow, label="Fairfax & Falls Church cities", color="#525252", size=5)+
...
annotate(geom="text", x=1.9, y=rappanhock_arrow, label="Rappahannock, Clarke, Warren & Culpeper Counties", color="#525252", size=5)+
...
annotate(geom="text", x=2.03, y=indep_cities_text, label="independent cities of\nFredericksburg, Manassas & Manassas Park", color="#525252", size=5)
...
###################### P1.G3. Plot Housing Units Percent change from 2000 to 2017 vs Median Housing Value 2000 ###################################################
...
g1 <- ggplot(data = df, aes(x = county_name_state_code, y = single_family_prop, fill=group)) +
  geom_bar(stat = "identity") +
  #ggtitle("Share Single Family Units, Housing Built 2000 or After") +
...
g2 <- ggplot(data = df, aes(x = county_name_state_code, y = total_new_units,fill=group)) +
  ...
  geom_bar(stat = "identity") +
  scale_y_continuous(labels = scales::comma, breaks = trans_breaks(identity, identity, n = 5))+
  #ggtitle("Number of Housing Units Built 2000 & After") +
  ggtitle("Number of Housing Units") +
  scale_fill_manual("Subject", values=colours)+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
# save the graph
ggsave(paste0(out_dir_ch03,"p3.g1_",dateo,"_acs_cnt_1950_2017_median_housing_value_household_income.jpg"),
       plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))


write.csv(df,paste0(out_dir_ch03,"p3.g1_",dateo,"_acs_cnt_1950_2017_median_housing_value_household_income.csv"),row.names = F)
...
p1 <- p1 +  annotate(geom="text", x=24, y=y_size, label="Urban", color="#045a8d", size=size_text)+
  annotate(geom="text", x=21, y=y_size, label="Suburban", color="#2b8cbe", size=size_text)+
  annotate(geom="text", x=16, y=y_size, label="Exurban", color="#74c476", size=size_text)
...
p1 <- p1 +  annotate(geom="text", x=24, y=y_pos, label="Urban", color="#045a8d", size=size_text)+
  annotate(geom="text", x=21, y=y_pos, label="Suburban", color="#2b8cbe", size=size_text)+
  annotate(geom="text", x=13, y=y_pos, label="Exurban", color="#74c476", size=size_text)
