...
# March 28, 2019
#
# acs_county_plots_ch01_ch03_v08.R
#
##############################################################################
##############################################################################
...
acs_cnt_2017_subset <- acs_cnt_1950_2017 %>%
  filter(year=='2017') %>%
  dplyr::select(total_single_family_households_since_2000,total_single_family_households_before_2000,
         total_2_to_4_family_households_since_2000,total_2_to_4_family_households_before_2000,
         total_5_to_19_family_households_since_2000, total_5_to_19_family_households_before_2000,
         total_20_to_49_family_households_since_2000,total_20_to_49_family_households_before_2000,
...
# melt the dataframe
acs_cnt_2017_subset_melt <- melt(acs_cnt_2017_subset, id.var=c("area_type"))
...
df <- acs_cnt_2017_area_wise_new_ext_housing_melt
df$group <- paste0(df$area_type, "-", df$unit_type, sep = "")

new_levels <- c("Exurban-2000 & After", "Exurban-Before 2000", "Suburban-2000 & After", "Suburban-Before 2000", "Urban-2000 & After", "Urban-Before 2000" )
...
# save the plot
ggsave(paste0(out_dir_ch01,"p1.g1_",dateo,"_acs_cnt_1950_2017_housing_units_area_wise.jpg"),
       plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))

write.csv(df,paste0(out_dir_ch01,"p1.g1_",dateo,"_acs_cnt_1950_2017_housing_units_area_wise.csv"),row.names = F)

###################### P1.G2. Plot Housing Units Area Wise - County Level ###################################################

# acs_cnt_2000_2017_comp <- acs_cnt_1950_2017 %>% filter(year=="2000" | year=="2017")
# 
# acs_cnt_2000_2017_comp_abs <- acs_cnt_2000_2017_comp %>%
...
# acs_cnt_2000_2017_comp_abs <- acs_cnt_2000_2017_comp_abs %>% arrange(year) %>% as.data.frame()
# 
# acs_cnt_2017_comp_abs <- acs_cnt_2000_2017_comp_abs %>% filter(year=="2017") %>% dplyr::select(county_name_state_code,area_type,housing_units_abs_change_from_2000)
# 
# # melt the dataframe
# acs_cnt_2017_subset_melt <- melt(acs_cnt_2017_comp_abs, id.var=c("county_name_state_code","area_type"))
# 
# colnames(acs_cnt_2017_subset_melt) <- c("county_name_state_code","area_type","unit_type","total_housing_units")
# 
# acs_cnt_2017_subset_melt$unit_type <- "2000 & After"
# 
# 
# 
# # filter for new housing units
# acs_cnt_2017_area_wise_new_housing_melt <- acs_cnt_2017_subset_melt
...
p1 <- ggplot(df, aes(x = area_type, y = total_housing_units, fill =  group)) +
  ...
  geom_text(data=df1,aes(label = county_name_state_code), # << move each label down by 1 unit
            position = position_stack(vjust=0.5), #vjust =2,
            color = "white", size = 6)+
geom_segment(aes(x = 1.3, y = fairfax_fall_church_arrow, xend = 1.55 , yend = fairfax_fall_church_arrow), arrow = arrow(length = unit(0.3, "cm")),color="#525252")+
...
ggsave(paste0(out_dir_ch01,"p1.g2_",dateo,"_acs_cnt_1950_2017_housing_units_After_2000_area_county_wise.jpg"),
       plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))

df$group <- NULL

write.csv(df,paste0(out_dir_ch01,"p1.g2_",dateo,"_acs_cnt_1950_2017_housing_units_After_2000_area_county_wise.csv"),row.names = F)
...
acs_cnt_2000_2017_comp_pct <- acs_cnt_2000_2017_comp %>%
  group_by(statefips, countyfips) %>%
  arrange(year, .by_group=TRUE) %>%
...
acs_cnt_2000_2017_comp_pct <- acs_cnt_2000_2017_comp_pct %>%
  group_by(statefips, countyfips) %>%
...
# arrange by year and make tibble as dataframe
acs_cnt_2000_2017_comp_pct <- acs_cnt_2000_2017_comp_pct %>% arrange(year) %>% as.data.frame()

acs_cnt_2000_2017_comp_pct_2000 <- subset(acs_cnt_2000_2017_comp_pct, year %in% c("2000"))

acs_cnt_2000_2017_comp_pct_2000$county_name <- gsub(" County","",acs_cnt_2000_2017_comp_pct_2000$county_name)


acs_cnt_2000_2017_comp_pct_2000_wo_Loudoun <- subset(acs_cnt_2000_2017_comp_pct, year %in% c("2000") & county_name!="Loudoun County")
...
p1 <- ggplot(data = acs_cnt_2000_2017_comp_pct_2000 , aes(x = median_housing_value, y = housing_units_pct_change_from_2000)) +
  geom_text_repel(aes(label=county_name, colour=factor(area_type)), size=7, fontface = "bold", segment.size = 0)+
  scale_x_continuous(labels = comma,limits= c(0, 450000), breaks = c(seq(0,450000,100000))) +
  labs(x = "Housing Value in 2000, $2017", y="Percentage change in number of housing units, 2000 to 2017")+
  scale_color_manual(values = c("Urban" = "#1f78b4","Suburban"="#a6cee3","Exurban"="#b2df8a"))+
  annotate(geom="text", x=409000, y=28, label="all counties", color="#bdbdbd", size=10)+
  annotate(geom="text", x=25000, y=45, label="without loudoun", color="#525252", size=10)+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
#c("#74c476","#2b8cbe","#045a8d")

# save the graph
ggsave(paste0(out_dir_ch01,"p1.g3_",dateo,"_acs_cnt_comp_2000_2017_scatter_house_median_value_percent_change_housing_units.jpg"),
       plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))

write.csv(acs_cnt_2000_2017_comp_pct_2000,
          paste0(out_dir_ch01,"p1.g3_",dateo,"_acs_cnt_comp_2000_2017_scatter_house_median_value_percent_change_housing_units.csv"),row.names = F)
...
# acs_cnt_2000_2017_comp_abs <- acs_cnt_2000_2017_comp_abs %>%
...
# 
# # plot the graph
# p1 <- ggplot(data = subset(acs_cnt_2000_2017_comp_abs, year %in% c("2000")), aes(x = median_housing_value,
...
#   labs(x = "median housing value 2000", y="% regional share in no. of housing units from 2000 to 2017")+
...
# 
# # save the graph
# ggsave(paste0(out_dir_ch01,"p1.g3.2_",dateo,"_acs_cnt_comp_2000_2017_scatter_house_median_value_regional_share_housing_units.jpg"),
#        plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))
...
acs_cnt_2017_new_ext_housing <- acs_cnt_1950_2017 %>%
                  filter(year=='2017') %>%
                  dplyr::select(county_name_state_code,area_type,total_single_family_households_since_2000,total_single_family_households_before_2000,
                         total_2_to_4_family_households_since_2000,total_2_to_4_family_households_before_2000,
                         total_5_to_19_family_households_since_2000, total_5_to_19_family_households_before_2000,
                         total_20_to_49_family_households_since_2000,total_20_to_49_family_households_before_2000,
                         total_50_or_more_family_households_since_2000,total_50_or_more_family_households_before_2000) %>%
...
#df$Unit_Type <- factor(df$Unit_Type,levels = c("2000 & After","Before 2000"))
...
p2 <- ggplot(df, aes(x = Units_Count, y = unit_freq, fill = Unit_Type)) +
  ...
  theme(panel.grid.major = element_blank(),
        ...
        legend.key = element_rect(fill = "white"),
        legend.spacing = unit(0.45,"cm"))
# geom_text(data = subset(df, Units_Count=="1 Unit"),
#           aes(label = Unit_Type), # << move each label down by 1 unit
...
p2 <- p2+annotate(geom="text", x=5.22, y=35, label="Built 2000 & after", color=col, size=siz)+
  annotate(geom="text", x=4.78, y=35, label="Built before 2000", color=col, size=siz)

# p2 <- p2+annotate(geom="text", x=5.22, y=35, label="Built before 2000", color=col, size=siz)+
#   annotate(geom="text", x=4.78, y=35, label="Built 2000 & after", color=col, size=siz)

p2

# save the graph
ggsave(paste0(out_dir_ch02,"p2.g1_",dateo,"_acs_cnt_1950_2017_housing_units_by_unit_type_and_count.jpg"),
       plot = p2, dpi = 300, width = 16, height = 11, units = c("in"))

write.csv(df,paste0(out_dir_ch02,"p2.g1_",dateo,"_acs_cnt_1950_2017_housing_units_by_unit_type_and_count.csv"),row.names = F)
...
######################### Box 2 Graphs ###################################################
box2_df <- read.csv(paste0(groupDir,"/center_for_washington_area_studies/data/box_data/box2_data.csv"))

box2_df$Region <- NULL

melt_box2 <- melt(box2_df,c("Variable"))

colnames(melt_box2) <- c("Variable","area_type","value")

melt_box2_share_oo_housing <- melt_box2 %>% filter(Variable=='Share of owner-occupied housing')

#melt_box2_share_housing <- melt_box2 %>% filter(Variable=='Share of all housing')

colours <- c("#1f78b4","#6baed6", "#74c476")


b1 <- ggplot(melt_box2_share_oo_housing, aes(x = area_type, y = value, fill = area_type)) +
  geom_bar(stat = "identity")+
  #scale_y_continuous(labels = scales::comma, breaks = trans_breaks(identity, identity, n = 5))+
  scale_fill_manual("Subject", values=colours)+
  labs(x = "", y = "% of owner-occupied housing", colour = "Parameter")+
  scale_shape_manual(values = c(16, 21)) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
ggsave(paste0(out_dir_ch02,"p2.b1_",dateo,"_share_owner_occupied_housing_area_wise.jpg"),
       plot = b1, dpi = 300, width = 16, height = 11, units = c("in"))

write.csv(melt_box2_share_oo_housing,paste0(out_dir_ch02,"p2.b1_",dateo,"_share_owner_occupied_housing_area_wise.csv"),row.names = F)



melt_box2_rowhouse_value <- melt_box2 %>% filter(Variable=='Value, rowhouse/other structures')

b2 <- ggplot(melt_box2_rowhouse_value, aes(x = area_type, y = value, fill = area_type)) +
  geom_bar(stat = "identity")+
  #scale_y_continuous(labels = scales::comma, breaks = trans_breaks(identity, identity, n = 5))+
  scale_fill_manual("Subject", values=colours)+
  labs(x = "", y = "Rowhouse value/Non-rowhouse value", colour = "Parameter")+
  scale_shape_manual(values = c(16, 21)) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
ggsave(paste0(out_dir_ch02,"p2.b2_",dateo,"_share_rowhouse_non_rowhouse_value.jpg"),
       plot = b2, dpi = 300, width = 16, height = 11, units = c("in"))

write.csv(melt_box2_rowhouse_value,paste0(out_dir_ch02,"p2.b2_",dateo,"_share_rowhouse_non_rowhouse_value.csv"),row.names = F)
...
p1 <- ggplot(df_all_years, aes(x=year)) +
  ...
  scale_y_continuous(labels = comma, breaks = c(0,100000,200000,300000,400000,500000,600000),limits = c(0,600000))+
  scale_x_continuous(breaks = c(1980, 1990, 2000, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017), 
                     labels = paste0(c("1980", "1990", "2000","2005", "2006", "2007", "2008", "2009", "2010", 
                                       "2011", "2012", "2013", "2014", "2015", "2016", "2017")))+
  scale_colour_manual(values = c(col_median_val,col_median_hh))+
  labs(y = "", x = "", colour = "Parameter")+
  scale_shape_manual(values = c(16, 21)) +
  annotate(geom="text", x=2007, y=600000, label="Median house value", color=col_median_val, size=12)+
  annotate(geom="text", x=2007, y=150000, label="Median household income", color=col_median_hh, size=12)+
  #labs(x="", y="") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
#   geom_line(aes(y=median_housing_value_yr,size=0.1, color="median house value")) +
#   geom_line(aes(y=median_household_income_yr,size=0.1, color="median household income")) +
#   scale_y_continuous(labels = comma, breaks = c(0,100000,200000,300000,400000,500000),limits = c(0,500000))+
#   scale_x_continuous(breaks = c(1980,1990,2000,2011,2017), labels = paste0(c("1980", "1990", "2000", "2011", "2017")))+
#   scale_colour_manual(values = c(col_median_val,col_median_hh))+
#   labs(y = "", x = "", colour = "Parameter")+
#   scale_shape_manual(values = c(16, 21)) +
#   annotate(geom="text", x=1990, y=350000, label="Median house value", color=col_median_val, size=12)+
#   annotate(geom="text", x=1990, y=125000, label="Median household income", color=col_median_hh, size=12)+
#   #labs(x="", y="") +
#   theme(panel.grid.major = element_blank(),
#         panel.grid.minor = element_blank(),
#         panel.background = element_blank(),
...
# save the graph
ggsave(paste0(out_dir_ch03,"p3.g1_",dateo,"_acs_cnt_2005_2017_median_housing_value_household_income.jpg"),
       plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))


write.csv(metro_value_income,paste0(out_dir_ch03,"p3.g1_",dateo,"_acs_cnt_2005_2017_median_housing_value_household_income.csv"),row.names = F)
...
y_size_urban <- df$median_housing_value_by_median_hh_income[x_urban]
y_size_suburban <- df$median_housing_value_by_median_hh_income[x_suburban]
y_size_exurban <- df$median_housing_value_by_median_hh_income[x_exurban]

size_text <- 12

p1 <- p1 +  annotate(geom="text", x=x_urban, y=y_size_urban+0.65, label="Urban", color="#045a8d", size=size_text)+
  annotate(geom="text", x=x_suburban, y=y_size_suburban+0.85, label="Suburban", color="#2b8cbe", size=size_text)+
  annotate(geom="text", x=x_exurban, y=y_size_exurban+0.8, label="Exurban", color="#74c476", size=size_text)

p1

ggsave(paste0(out_dir_ch03,"p3.g2_",dateo,"_acs_cnt_2017_","median_housing_value_by_median_hh_income",".jpg"),
       plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))

write.csv(df,paste0(out_dir_ch03,"p3.g2_",dateo,"_acs_cnt_2017_","median_housing_value_by_median_hh_income",".csv"),row.names = F)
###################### P3.G2.2 Plot Housing Units Percent change from 2000 to 2017 vs Median Housing Value 2000 ###################################################
...
# 
# # plot the graph
# p1 <- ggplot(data = subset(acs_cnt_1950_2017, year %in% c("2017")), aes(x = median_housing_value, y = median_household_income)) +
...
#   labs(x = "median housing value 2017", y="median household income 2017")+
#   scale_color_manual(values = c("Urban" = "#1f78b4","Suburban"="#a6cee3","Exurban"="#b2df8a"))+
#   stat_smooth(method="lm_right", fullrange=TRUE,col='#bdbdbd', se=FALSE) +
#   theme(panel.grid.major = element_blank(),
#         panel.grid.minor = element_blank(),
...
# 
# # save the graph
# ggsave(paste0(out_dir_ch03,"p3.g2.2_",dateo,"_acs_cnt_2017_scatter_house_median_value_vs_median_household_income.jpg"),
#        plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))
# 
# 
# write.csv(subset(acs_cnt_1950_2017, year %in% c("2017")),
#           paste0(out_dir_ch03,"p3.g2.2_",dateo,"_acs_cnt_2017_scatter_house_median_value_vs_median_household_income.csv"),row.names=F)
...
y_pos_urban <- df$median_gross_rent_as_a_percentage_of_household_income[x_urban]
y_pos_suburban <- df$median_gross_rent_as_a_percentage_of_household_income[x_suburban]
y_pos_exurban <- df$median_gross_rent_as_a_percentage_of_household_income[x_exurban]

size_text <- 12

p1 <- p1 +  annotate(geom="text", x=x_urban, y=y_pos_urban+3.5, label="Urban", color="#045a8d", size=size_text)+
  annotate(geom="text", x=x_suburban, y=y_pos_suburban+4.5, label="Suburban", color="#2b8cbe", size=size_text)+
  annotate(geom="text", x=x_exurban, y=y_pos_exurban+4, label="Exurban", color="#74c476", size=size_text)

p1

ggsave(paste0(out_dir_ch03,"p3.g3_",dateo,"_acs_cnt_2017_","median_gross_rent_as_a_percentage_of_household_income",".jpg"),
       plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))

write.csv(df,paste0(out_dir_ch03,"p3.g3_",dateo,"_acs_cnt_2017_","median_gross_rent_as_a_percentage_of_household_income",".csv"),row.names = F)

####################### Introduction Graph ##########################

acs_cnt_1950_2017_decade <- acs_cnt_1950_2017 %>% filter(year %in% c("1990","2000","2010","2017"))

acs_cnt_1950_2017_pop_housing <- acs_cnt_1950_2017_decade %>%
                                    group_by(year) %>%
                                    summarise_at(c("total_population","total_housing_units"), funs(sum), na.rm=TRUE)


acs_cnt_1950_2017_pop_housing <- acs_cnt_1950_2017_pop_housing %>%
  mutate(Population_growth =((total_population/lag(total_population,1))-1)*100) %>%
  mutate(Housing_growth =((total_housing_units/lag(total_housing_units,1))-1)*100) %>%
  as.data.frame()
...
acs_cnt_1950_2017_pop_housing_not_NA <- na.omit(acs_cnt_1950_2017_pop_housing)

acs_cnt_1950_2017_pop_housing_not_NA$year <- c("1990-2000","2000-2010","2010-2017")

acs_cnt_1950_2017_pop_housing_not_NA <- acs_cnt_1950_2017_pop_housing_not_NA %>% dplyr::select(year,Population_growth,Housing_growth)

acs_cnt_1950_2017_pop_housing_not_NA_melt <- melt(acs_cnt_1950_2017_pop_housing_not_NA,id.vars = "year")
...
ColourPalleteMulti <- function(df, group, subgroup){
  ...
  category.start <- c("#8856a7","#8856a7","#8856a7") # Set the top of the colour pallete
  category.end  <- c("#810f7c","#810f7c","#810f7c") # set the bottom #74c476
...
# Create data
df <- acs_cnt_1950_2017_pop_housing_not_NA_melt
df$group <- paste0(df$year, "-", df$variable, sep = "")

# Build the colour pallete
colours <-ColourPalleteMulti(df, "year", "variable")
...
df$year <- factor(df$year, levels = rev(unique(df$year)))

p <- ggplot(df, aes(x = year, y = value, fill =  group)) +
  geom_bar(position = "dodge", stat = "identity")+
  #scale_y_continuous(labels = scales::comma, breaks = trans_breaks(identity, identity, n = 4))+
  labs(x = "", y = "", colour = "Parameter")+
  scale_fill_manual("Subject", values=colours)+
  theme(panel.grid.major = element_blank(),
        ...
        legend.key = element_rect(fill = "white"),
        legend.spacing = unit(0.45,"cm"))#+
  # geom_text(data=subset(df,year=="1990-2000"),
  #           aes(label = variable), # << move each label down by 1 unit
...
# 
# 
p1 <- p+coord_flip()+annotate(geom="text", x=2.8, y=7.5, label="Housing growth", color=col, size=siz)+
  annotate(geom="text", x=3.2, y=7.5, label="Population growth", color=col, size=siz)
...
# save the graph
ggsave(paste0(out_dir_intro,"i1.g1_",dateo,"_acs_cnt_1950_2017_population_and_housing_growth.jpg"),
       plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))


write.csv(df,paste0(out_dir_intro,"i1.g1_",dateo,"_acs_cnt_1950_2017_population_and_housing_growth.csv"),row.names = F)


# plot the graph
# p1 <- ggplot(acs_cnt_1950_2017_pop_housing, aes(x=year, group=1)) +
...
#   scale_x_discrete(breaks = c(1950,1960,1970,1980,1990,2000,2010,2017),
#                    labels = paste0(c("1950","1960","1970","1980", "1990", "2000", "2010","2017")))+
...
#col_name <- "total_population"
#df <- acs_cnt_1950_2017

generate_tables <- function(df,col_name){

df <- df %>% dplyr::select("county_name", "state_name", "year", col_name)

#df[[col_name]] <- lapply(df[[col_name]], change_scientific)

df_cast <- dcast(df, county_name+state_name ~ year, value.var = col_name) %>% arrange(state_name) 

df_cast_yrs <- df_cast %>% dplyr::select(county_name,state_name,"2017","2010","2000")

val_total <- colSums(df_cast_yrs[,c(3,4,5)])

df_total <- data.frame(c("Capital Region"),c("Capital Region"),c(val_total[1]),c(val_total[2]),c(val_total[3]))

df_dc <- subset(df_cast_yrs,state_name=="District of Columbia")

df_md <- subset(df_cast_yrs,state_name=="Maryland")

df_va <- subset(df_cast_yrs,state_name=="Virginia")

df_wv <- subset(df_cast_yrs,state_name=="West Virginia")

df_row_md <- data.frame(c("Maryland"),c(""),c(""),c(""),c(""))

df_row_va <- data.frame(c("Virginia"),c(""),c(""),c(""),c(""))

df_row_wv <- data.frame(c("West Virginia"),c(""),c(""),c(""),c(""))

names(df_total) <- names(df_cast_yrs)
names(df_row_md) <- names(df_cast_yrs)
names(df_row_va) <- names(df_cast_yrs)
names(df_row_wv) <- names(df_cast_yrs)


cols <- colnames(df_dc)[3:5]

df_total[cols] <- lapply(df_total[cols], change_scientific)
df_dc[cols] <- lapply(df_dc[cols], change_scientific)
df_md[cols] <- lapply(df_md[cols], change_scientific)
df_va[cols] <- lapply(df_va[cols], change_scientific)
df_wv[cols] <- lapply(df_wv[cols], change_scientific)

# df_dc[cols] <- lapply(df_dc[cols], change_character)
# df_md[cols] <- lapply(df_md[cols], change_character)
# df_va[cols] <- lapply(df_va[cols], change_character)
# df_wv[cols] <- lapply(df_wv[cols], change_character)


df_table <- rbind(df_total,df_dc,df_row_md,df_md,df_row_va,df_va,df_row_wv,df_wv)

df_table$state_name <- NULL




if(col_name=="total_population"){
colnames(df_table) <- c("Name","Population\n2017","Population\n2010", "Population\n2000")
write.csv(df_table,paste0(out_dir_appendix,"t1.",dateo,"_",col_name,".csv"),row.names = F)
}else{
  colnames(df_table) <- c("Name","Housing\n2017","Housing\n2010", "Housing\n2000")
  write.csv(df_table,paste0(out_dir_appendix,"t2.",dateo,"_",col_name,".csv"),row.names = F)
}


}

col_vec <- c("total_population", "total_housing_units")


for (col in col_vec){
  generate_tables(acs_cnt_1950_2017,col)
}
...
acs_cnt_2017 %>% select(.,matches("*_2000$")) 

acs_cnt_2017 <- acs_cnt_2017 %>% mutate("total_households_2000_before_after"=dplyr::select(.,matches("*_2000$")) %>% apply(1, sum, na.rm=TRUE))

acs_cnt_2017 <- acs_cnt_2017 %>% mutate("% sf"=round((total_single_family_households_since_2000+total_single_family_households_before_2000)/
                                                       total_households_2000_before_after*100,1))

acs_cnt_2017 <- acs_cnt_2017 %>% mutate("% oo" = round(total_owner_occupied/total_housing_units*100,1))

df_cast_yrs <- acs_cnt_2017 %>% select(county_name, state_name, `% sf`,`% oo`, median_housing_value)
...
df_table <- rbind(df_dc,df_row_md,df_md,df_row_va,df_va,df_row_wv,df_wv)
...
colnames(df_table) <- c("Name","Share Single-Family\n2017","Share Owner-Occupied\n2017", "Median Home Value\n2017")

print(df_table)

write.csv(df_table,paste0(out_dir_appendix,"t3.",dateo,"_sf_oo_median_home_value",".csv"),row.names = F)
