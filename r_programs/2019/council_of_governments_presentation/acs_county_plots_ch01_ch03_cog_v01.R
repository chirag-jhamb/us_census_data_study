...
#
# acs_county_plots_ch01_ch03_cog_v01.R
#
##############################################################################
##############################################################################
...
data_dir <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/python_output/2019/council_of_governments/data_directory/")
...
p1
# save the plot
ggsave(paste0(out_dir_ch01,"p1.g2_",dateo,"_.jpg"),
       plot = p1, dpi = 300, width = 11, height = 8, units = c("in"))
...
p1 <- ggplot(data = acs_cnt_2000_2017_comp_pct_2000 , aes(x = median_housing_value, y = housing_units_pct_change_from_2000)) +
  ...
  labs(x = "median housing value, 2000", y="percentage change in number of housing units, 2000 to 2017")+
...
# save the graph
ggsave(paste0(out_dir_ch01,"p1.g3_",dateo,".jpg"),
       plot = p1, dpi = 300, width = 11, height = 8, units = c("in"))
...
g.mid<-ggplot(df,aes(x=1,y=county_name_state_code))+
  geom_text(aes(label=county_name_state_code),size=7,color="#525252")+
...
p1 <- ggplot(df, aes(x=year)) +
  ...
  scale_y_continuous(labels = comma, breaks = c(0,100000,200000,300000,400000),limits = c(0,450000))+
