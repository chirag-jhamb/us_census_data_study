...
# dmv_subset$COUNTY <- sapply(dmv_subset$COUNTY, as.character)
...
dmv_subset_graph <- dmv_subset_graph %>% group_by(area_type, year) %>% summarise(total_population = sum(total_population))
dmv_subset_graph <- as.data.frame(dmv_subset_graph)
# p = ggplot(dmv_subset_graph, aes(x=year, y=total_population, fill=area_type)) + geom_area(position = 'stack')
# dmv_subset_graph$area_type <- as.vector(dmv_subset_graph$area_type)

p <- ggplot(dmv_subset_graph, aes( year, as.numeric(total_population),fill= as.factor(area_type)))
p <- p + geom_area(position = 'stack')
dateo <- paste(substr(Sys.Date(),1,4),substr(Sys.Date(),6,7),substr(Sys.Date(),9,10),sep="")
dateo
groupDir <- "/groups/brooksgrp"
out_dir_intro <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2020/2020_report/ch01/g2")
save_path <-paste0(out_dir_intro,dateo,".jpg")
