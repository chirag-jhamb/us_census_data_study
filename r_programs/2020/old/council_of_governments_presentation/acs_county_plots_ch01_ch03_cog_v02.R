...
# March 12, 2019
#
# acs_county_plots_ch01_ch03_cog_v02.R
#
##############################################################################
##############################################################################
...
p1 <- ggplot(df, aes(x = area_type, y = total_housing_units, fill =  group)) +
  ...
  geom_text(data=df1,aes(label = county_name_state_code), # << move each label down by 1 unit
            position = position_stack(vjust=0.5), #vjust =2,
            color = "white", size = 6)+
  annotate(geom="text", x=1.1, y=fairfax_fall_church_arrow, label="Fairfax & Falls Church cities", color="#525252", size=5)
...
p1
# save the plot
ggsave(paste0(out_dir_ch01,"p1.g2_",dateo,".jpg"),
       plot = p1, dpi = 300, width = 11, height = 8, units = c("in"))
...
p1 <- ggplot(data = acs_cnt_2000_2017_comp_pct_2000 , aes(x = median_housing_value, y = housing_units_pct_change_from_2000)) +
  ...
  annotate(geom="text", x=430000, y=26, label="all counties", color="#bdbdbd", size=7)+
  annotate(geom="text", x=30000, y=45, label="without loudoun", color="#525252", size=7)+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
# save the graph
ggsave(paste0(out_dir_ch02,"p2.g1_",dateo,".jpg"),
...
# save the graph
ggsave(paste0(out_dir_ch02,"p2.g2_",dateo,".jpg"),
...
p1 <- ggplot(df, aes(x=year)) +
  ...
  scale_shape_manual(values = c(16, 21)) +
  annotate(geom="text", x=1990, y=340000, label="Median house value", color=col_median_val, size=12)+
  annotate(geom="text", x=1990, y=120000, label="Median household income", color=col_median_hh, size=12)+
  #labs(x="", y="") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
# save the graph
ggsave(paste0(out_dir_ch03,"p3.g1_",dateo,".jpg"),
       plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))
...
ggsave(paste0(out_dir_ch03,"p3.g2_",dateo,".jpg"),
       plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))
