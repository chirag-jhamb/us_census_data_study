...
out_dir <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/python_output/2019/")


# load the data
acs_cnt_2012_2016 <- read.csv(paste0(data_dir,"20181212_cnty_acs_2012_2016_absolute_values.csv"),stringsAsFactors = F)
...
data %>% select(intersect(starts_with('Built 20'),contains(':'))) %>% head(2)
...
data %>% select(intersect(starts_with('Built 19'),contains(':'))) %>% head(2)
...
county_area <- st_read("/groups/brooksgrp/maps/united_states/census2010/counties",layer="cnty_2010_20140313",stringsAsFactors = F)
...
county_area %>% filter(paste0(STATE,COUNTY) %in% state_county) %>% select(STATE,COUNTY,CENSUSAREA)
########
...
#dd %>% mutate('single_family_households'=select(.,matches('^1, detached.*attached$|attached.1$|attached.6$|attached.7$')) %>% apply(1, sum, na.rm=TRUE))
#dd %>% select(setdiff(starts_with('1, detached'),matches('attached$|attached.1$|attached.6$|attached.7$'))) %>% head()
...
#data %>% select(-matches('^1, detached.*attached$|attached.1$|attached.6$|attached.7$')) %>% head()
#dd %>% mutate('single'=select(.,matches('^1, detached.*attached$|attached.1$|attached.6$|attached.7$')) %>% apply(1, sum, na.rm=TRUE))
#dd %>% select(setdiff(starts_with('1, detached'),matches('attached$|attached.1$|attached.6$|attached.7$'))) %>% head()
#data %>% mutate('multiple'=select(.,setdiff(starts_with('1, detached'),matches('attached$|attached.1$|attached.6$|attached.7$')))%>% apply(1, sum, na.rm=TRUE))
