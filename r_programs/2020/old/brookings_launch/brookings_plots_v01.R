...
p1 <- ggplot(df, aes(x = area_type, y = total_housing_units, fill =  group)) +
  ...
  geom_text(data=df1,aes(label = county_name_state_code), # << move each label down by 1 unit
            ...
            color = "white", size = 6) +
  annotate(geom="text", x=1.45, y=fairfax_fall_church_arrow + 10000, label="Fairfax &", 
  	   color="#525252", size=anno_text_size,
	   hjust = 1) +
  annotate(geom="text", x=1.45, y=fairfax_fall_church_arrow, label="Falls Church cities", 
  	   color="#525252", size=anno_text_size,
	   hjust = 1)
