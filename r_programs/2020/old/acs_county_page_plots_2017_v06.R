...
# April 10, 2019
#
# acs_county_page_plots_2017_v06.R
#
##############################################################################
##############################################################################
...
out_dir <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2019/county_pages/")
...
# create directories to save the figures (if diectory exists it does nothing)
create_dir <- function(out_dir,sfips,cfips){
  
  out_dir_c <- paste0(out_dir,sfips,cfips,"/")
  
  sys_command <- paste0("mkdir -p  ",out_dir_c)
  
  sys_command_d <- paste0(sys_command,"data/")
  
  system(sys_command)
  
  system(sys_command_d)
  
}

#https://stackoverflow.com/questions/8790515/how-to-check-directory-exist-or-not-in-linux/8790538
...
for (sfips in state_fips){
  ...
  if(sfips=="00"){
    cfips <-  c("000")
    create_dir(out_dir,sfips,cfips)
  } else if(sfips=="11"){
    cfips <-  c("001")
    create_dir(out_dir,sfips,cfips)
  } else if (sfips=="51"){
...
acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25034_([4-7]|1[2-9])_')) %>% head(2)

#acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25034_2|B25034_3')) %>% head(2)


acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
                      mutate('B25034_12_built_2010_to_2017'=dplyr::select(.,matches('^B25034_2|B25034_3')) %>%
                            apply(1, sum, na.rm=TRUE))


acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
  mutate('B25034_13_built_1949_or_earlier'=dplyr::select(.,matches('^B25034_10|B25034_11')) %>%
           apply(1, sum, na.rm=TRUE))

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
  mutate('B25034_14_built_1950_to_1969'=dplyr::select(.,matches('^B25034_8|B25034_9')) %>%
           apply(1, sum, na.rm=TRUE))

################# create columns for structure type -- Graph 2 ################
#acs_cnt_2013_2017_subset %>% dplyr::select(intersect(starts_with("B25127"),contains("2_to_4"))) %>% head(2)

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
                      mutate('1_Units'=dplyr::select(.,intersect(starts_with("B25127"),contains("1,_detached"))) %>%
                                   apply(1, sum, na.rm=TRUE))

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
                            mutate('2_to_4_Units'=dplyr::select(.,intersect(starts_with("B25127"),contains("2_to_4"))) %>%
                                     apply(1, sum, na.rm=TRUE))

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
                                    mutate('5_to_19_Units'=dplyr::select(.,intersect(starts_with("B25127"),contains("5_to_19"))) %>%
                                             apply(1, sum, na.rm=TRUE))


acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
                            mutate('20_to_49_Units'=dplyr::select(.,intersect(starts_with("B25127"),contains("20_to_49"))) %>%
                                     apply(1, sum, na.rm=TRUE))


acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
                              mutate('50_or_more_Units'=dplyr::select(.,intersect(starts_with("B25127"),contains("50_or_more"))) %>%
                                       apply(1, sum, na.rm=TRUE))

################# create columns for owner occupied homes -- Graph 3 ################
acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25075')) %>% head(2)

acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25075_([2-9]|[1][0-8])_')) %>% head(2)

acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25075_([1][9]|[2][0])_')) %>% head(2)

acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25075_([2][4-7])_')) %>% head(2)

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
  mutate('B25075_28_less_than_$200,000'=dplyr::select(.,matches('^B25075_([2-9]|[1][0-8])_')) %>%
           apply(1, sum, na.rm=TRUE))

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
 mutate('B25075_29_$200,000_to_$299,999'=dplyr::select(.,matches('^B25075_([1][9]|[2][0])_')) %>%
           apply(1, sum, na.rm=TRUE))

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
  mutate('B25075_30_$750,000_or_more'=dplyr::select(.,matches('^B25075_([2][4-7])_')) %>%
           apply(1, sum, na.rm=TRUE))

acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25075_([2][1-3]|[2][8-9]|[3][0])_')) %>% colnames()

################# create columns for structure type -- Graph 4 ################
acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B19001')) %>% colnames()

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
            mutate('B19001_18_less_than_$40,000'=dplyr::select(.,matches('^B19001_([2-8])_')) %>%
                apply(1, sum, na.rm=TRUE))

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
  mutate('B19001_19_$40,000_to_$74,999'=dplyr::select(.,matches('^B19001_([9]|[1][0-2])_')) %>%
           apply(1, sum, na.rm=TRUE))

# acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
#   mutate('B19001_20_$30,000_to_$39,999'=dplyr::select(.,matches('^B19001_7|B19001_8')) %>%
#            apply(1, sum, na.rm=TRUE))


acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
  mutate('B19001_20_$100,000_to_$149,999'=dplyr::select(.,matches('^B19001_14|B19001_15')) %>%
           apply(1, sum, na.rm=TRUE))

acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B19001_([1][3]|[1][6-9]|[2][0])')) %>% colnames()

################ create columns for structure type -- Graph 5 ################
acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B03002_3_|B03002_4_|B03002_6_|B03002_12_'))


acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
                                mutate('total_white_hispanic_black_asian'=dplyr::select(.,matches('^B03002_3_|B03002_4_|B03002_6_|B03002_12_')) %>%
                                apply(1, sum, na.rm=TRUE))
...
acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25063')) %>% colnames()

acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25063_([3-9]|[1][0-7])')) %>% colnames()

#acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25063_([1][8-9])')) %>% colnames()

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
  mutate('B25063_28_less_than_$1,000'=dplyr::select(.,matches('^B25063_([3-9]|[1][0-9])')) %>%
           apply(1, sum, na.rm=TRUE))

#acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
 # mutate('B25063_29_$600_to_$799'=dplyr::select(.,matches('^B25063_([1][4-7])')) %>%
  #         apply(1, sum, na.rm=TRUE))

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
  mutate('B25063_29_$1,000_to_$1,499'=dplyr::select(.,matches('^B25063_([2][0-1])')) %>%
           apply(1, sum, na.rm=TRUE))

acs_cnt_2013_2017_subset <- acs_cnt_2013_2017_subset %>% 
  mutate('B25063_30_$2,500_or_more'=dplyr::select(.,matches('^B25063_([2][4-6])')) %>%
           apply(1, sum, na.rm=TRUE))


acs_cnt_2013_2017_subset %>% dplyr::select(matches('^B25063_([2][2-3]|[2][7-9]|[3][0-9])_')) %>% colnames()
...
acs_cnt_2013_2017_subset_built_year <- acs_cnt_2013_2017_subset %>%
                                        dplyr::select(STATE_FIPS,county_code,county_name,matches('^B25034_([4-7]|1[2-9])_')) %>%
                                        as.data.frame()

#built_cols <- colnames(acs_cnt_2013_2017_subset_built_year)[4:ncol(acs_cnt_2013_2017_subset_built_year)] 

built_cols <- c("2000 to 2009","1990 to 1999" ,"1980 to 1989", "1970 to 1979", "2010 to 2017", "before 1950" , "1950 to 1969")
...
acs_cnt_2013_2017_subset_built_year_melt$variable <- factor(acs_cnt_2013_2017_subset_built_year_melt$variable, 
                                                                  c("before 1950",  "1950 to 1969", "1970 to 1979", "1980 to 1989", 
...
out_dir_c <- paste0(out_dir,statefips,countyfips,"/")
#print(p1)
# save the graph
ggsave(paste0(out_dir_c,"c1.g1.",statefips,countyfips,"_",dateo,".jpg"),
...
out_dir_d <- paste0(out_dir_c,"data/")

write.csv(df,paste0(out_dir_d,"c1.g1.",statefips,countyfips,"_",dateo,".csv"),row.names = F)
...
acs_cnt_2013_2017_subset_structure_type <- acs_cnt_2013_2017_subset %>%
                                dplyr::select(STATE_FIPS,county_code,county_name,contains("_Units")) %>%
                                              as.data.frame()
...
#print(p1)
# save the graph
ggsave(paste0(out_dir_c,"c1.g2.",statefips,countyfips,"_",dateo,".jpg"),
       plot = p1, dpi = dpi_val, width = width_val, height = height_val, units = c("in"))
...
write.csv(df,paste0(out_dir_d,"c1.g2.",statefips,countyfips,"_",dateo,".csv"),row.names = F)
...
acs_cnt_2013_2017_subset_owner_occupied <- acs_cnt_2013_2017_subset %>%
                                     dplyr::select(STATE_FIPS,county_code,county_name,matches("^B25075_([2][1-3]|[2][8-9]|[3][0])_")) %>%
                                               as.data.frame()
...
#print(p1)
# save the graph
ggsave(paste0(out_dir_c,"c1.g3.",statefips,countyfips,"_",dateo,".jpg"),
       plot = p1, dpi = dpi_val, width = width_val, height = height_val, units = c("in"))
...
write.csv(df,paste0(out_dir_d,"c1.g3.",statefips,countyfips,"_",dateo,".csv"),row.names = F)
...
acs_cnt_2013_2017_subset_household_income <- acs_cnt_2013_2017_subset %>%
                                            dplyr::select(STATE_FIPS,county_code,county_name,matches('^B19001_([1][3]|[1][6-9]|[2][0])')) %>%
                                                    as.data.frame()
#
...
#print(p1)
# save the graph
ggsave(paste0(out_dir_c,"c1.g4.",statefips,countyfips,"_",dateo,".jpg"),
       plot = p1, dpi = dpi_val, width = width_val, height = height_val, units = c("in"))
...
write.csv(df,paste0(out_dir_d,"c1.g4.",statefips,countyfips,"_",dateo,".csv"),row.names = F)
...
acs_cnt_2013_2017_subset_race <- acs_cnt_2013_2017_subset %>%
                                               dplyr::select("STATE_FIPS","county_code","county_name",matches('^B03002_3_|B03002_4_|B03002_6_|B03002_12_'),
                                                      "All_Others") %>%
                                                 as.data.frame()
...
#print(p1)
# save the graph
ggsave(paste0(out_dir_c,"c1.g5.",statefips,countyfips,"_",dateo,".jpg"),
       plot = p1, dpi = dpi_val, width = 19, height = 15, units = c("in"))
...
write.csv(df,paste0(out_dir_d,"c1.g5.",statefips,countyfips,"_",dateo,".csv"),row.names = F)
...
acs_cnt_2013_2017_subset_household_type <- acs_cnt_2013_2017_subset %>%
                                             dplyr::select(STATE_FIPS,county_code,county_name,matches('^B25063_([2][2-3]|[2][7-9]|[3][0-9])_')) %>%
                                                                    as.data.frame()
...
#print(p1)
# save the graph
ggsave(paste0(out_dir_c,"c1.g6.",statefips,countyfips,"_",dateo,".jpg"),
       plot = p1, dpi = dpi_val, width = width_val, height = height_val, units = c("in"))
...
write.csv(df,paste0(out_dir_d,"c1.g6.",statefips,countyfips,"_",dateo,".csv"),row.names = F)
...
######################### Create data table for region code and name #########################

df_key <- acs_cnt_2013_2017_subset %>% dplyr::select(STATE_FIPS,county_code,county_name,state_code)

df_key <- df_key %>% mutate(region_name=str_c(county_name,", ",state_code)) %>% as.data.frame()

df_key <- df_key %>% mutate(region_code=str_c(STATE_FIPS,"",county_code)) %>% as.data.frame()

df_key <- df_key %>% dplyr::select(region_code,region_name)

df_key$region_name <- ifelse(df_key$region_name=='District of Columbia, DC','Washington, DC',df_key$region_name)

df_metro <- data.frame(c("00000"),c("Metro Area"))

colnames(df_metro) <- colnames(df_key)

df_key_table <- rbind(df_metro,df_key)

out_dir_key <- paste0(out_dir,"county_ID_table/")

system(paste0("mkdir -p ",out_dir_key))

write.csv(df_key_table,paste0(out_dir_key,"key_region_code_name",".csv"),row.names = F)
