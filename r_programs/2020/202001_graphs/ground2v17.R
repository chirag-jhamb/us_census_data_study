...
if(inccln){
  ...
  bg1 <- read_csv("/groups/brooksgrp/center_for_washington_area_studies/state_of_the_capitol_region/python_output/2020/block_group_data/20200122_block_group_tract_data.csv",
      	   col_types = cols(
  ...
  # that above should work, but i think chirag messed up the years
  bg1$year <- 2013
...
if(incpop){
  ...
  bgsc1990 <- bgsc[which(bgsc$year == 1990),]
  bgsc1990 <- filter(.data = bgsc1990, as.numeric(pop.den.1000) < 50)

  names(bgsc1990)

  # make population density scatter
  pden <- ggplot(bgsc1990) +
       geom_point(mapping = aes(x = bg_med_inc, y = as.numeric(pop.den.1000)))

  # make population density scatter w/ log
  lpden <- ggplot(bgsc1990) +
       geom_point(mapping = aes(x = bg_med_inc, y = as.numeric(ln.pop.den)))

  ### output graph
  nm <- paste0(janmtg,
     	         "income/by_pop_density/y1990_inc_vs_popd_",
               dateo,
	       ".jpg")
  ...
  ### output graph
  nm2 <- paste0(janmtg,
     	         "income/by_pop_density/y1990_inc_vs_ln_popd_",
               dateo,
	       ".jpg")
