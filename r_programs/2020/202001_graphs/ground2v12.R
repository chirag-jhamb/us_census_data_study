...
if(inccln){
  ...
  cpis <- filter(.data = cpi, series_id == "CUSR0000SA0" & substr(as.character(period),1,1) == "M")
  print(head(cpis))
  ...
  cpi.annual$o2018 <- ifelse(cpi.annual$year == 2018, cpi.ann, NA)
  cpi.annual$v2018 <- mutate(.data = cpi.annual, v2018 = max(o2018, na.rm. = TRUE))
  print(cpi.annual)
  ...
  bg1 <- read.csv("/groups/brooksgrp/center_for_washington_area_studies/state_of_the_capitol_region/python_output/2020/block_group_data/20191217_block_group_tract_data.csv")
...
### shoot - these need land area variables
  ...
  bg9020 <- read_sas("/groups/brooksgrp/center_for_washington_area_studies/state_of_the_capitol_region/sas_output/2020/dec_block_grp_census/bgs_1990_2000_dmv_20191212.sas7bdat")
