...
# january  20, 2020 .. trying to add block group size to income data
# january  21, 2020 .. more of same
# january  22, 2020 .. with fixed population data for block groups
# january  23, 2020 .. and more cleaning up to merge program i did on my hard drive with the pegasus one
#
# ground2v19.R 
#
##################################################################
...
# median income dbns
incg <- FALSE
# median income, full msa and by urban/sub/ex
incg2 <- FALSE
# median income, by population growth over these years
incg3 <- FALSE
...
if(ploc == "pegasus"){
  ...
  # location for original county data
  county.data.dir <- "/groups/brooksgrp/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2020/20191101_meeting/"
  ...
  # location for cpi data 
  cpidats <- "/groups/brooksgrp/consumer_price_index/all_urban_consumers/through_2018/cu.data.1.AllItems.txt"
  
  # block group data
  bgdats <- "/groups/brooksgrp/center_for_washington_area_studies/state_of_the_capitol_region/python_output/2020/block_group_data/20191217_block_group_tract_data.csv"
  
  # old block group data
  oldbgdats <- "/groups/brooksgrp/center_for_washington_area_studies/state_of_the_capitol_region/sas_output/2020/dec_block_grp_census/bgs_1990_2000_dmv_20191219.sas7bdat"
...
if(ploc == "hdrive"){
  ...
  # location for county data
  county.data.dir <- "h:/center_for_washington_area_studies/state_of_the_capital_region/2019-2020/r_output/20191101_meeting/"
  
  # location for january meeting output
  janmtg <- "h:/center_for_washington_area_studies/state_of_the_capital_region/2019-2020/r_output/202001_meeting/"

  # cpi data
  cpidats <- "h:/center_for_washington_area_studies/state_of_the_capital_region/2019-2020/r_output/cpi/cu.data.1.AllItems.txt"

  # block group data
  bgdats <- "h:/center_for_washington_area_studies/state_of_the_capital_region/2019-2020/python_output/block_group_data/20191220_block_group_tract_data.csv"

  # old block group data
  oldbgdats <- "h:/center_for_washington_area_studies/state_of_the_capital_region/2019-2020/sas_output/bgs_1990_2000_dmv_20191219.sas7bdat"
...
if(cdata){
  ...
  # changed by chirag from 20191211 to 20191212
    orgd.nm <- paste0(county.data.dir,"20191212_dataset_all_years.csv")
  ...
  bgsc$area_type <- 
    ifelse(  bgsc$statefips == "11" |
      ...
      ifelse((bgsc$statefips == "24" & bgsc$countyfips %in% c("033","031")) |
        ...
        ifelse((bgsc$statefips == "24" & bgsc$countyfips %in% c("009","017","021")) |			       (bgsc$statefips == "51" & bgsc$countyfips %in% c("043","047","059","061","069",
			       	       	 		   "107","153","157","177","179", 
                                                   	   "187","600","610","630",
							   "683","685","840")) |               
...
### income and population density graphs #####

if(incpop){

  # make log of population density
  bgsc$ln.pop.den <- ifelse(as.numeric(bgsc$pop.den.1000) > 0 & 
  		            is.na(bgsc$pop.den.1000) == FALSE, 
  		       log(as.numeric(bgsc$pop.den.1000)),NA)
  summary(bgsc$ln.pop.den)

  ## make a function to run through years 
  rfunc <- function(yearso){

    # make a subset for 1990
    bgsc1990 <- bgsc[which(bgsc$year == yearso),]
    bgsc1990 <- filter(.data = bgsc1990, as.numeric(pop.den.1000) < 50)

    # make a title 
    tito <- paste0(yearso, ": median income vs population density * 1000")

    names(bgsc1990)

    # make population density scatter
    pden <- ggplot(bgsc1990, mapping = aes(x = bg_med_inc_real, y = as.numeric(pop.den.1000))) +
       geom_point(mapping = aes(x = bg_med_inc_real, y = as.numeric(pop.den.1000))) +
       geom_smooth(method='lm', formula= y ~ x) +
       labs(title = "real median income over time dc",
           x = "real block group median income",
           y = "population density per m2 * 1000")

    # make population density scatter w/ log
    lpden <- ggplot(bgsc1990, mapping = aes(x = bg_med_inc_real, y = as.numeric(ln.pop.den))) +
       geom_point(mapping = aes(x = bg_med_inc_real, y = as.numeric(ln.pop.den))) +
       geom_smooth(method='lm', formula= y ~ x) +
       labs(title = "real median income over time dc",
           x = "real block group median income",
           y = "log of population density per m2 * 1000")

    ### output graph
    nm <- paste0(janmtg,
     	         "income/by_pop_density/y",
		 yearso,
		 "_inc_vs_popd_",
               dateo,
	       ".jpg")
  ...
  } # end of function to go through years

  ### call the function 

  ## make a list of years
  yr.list <- unique(bgsc$year)

  ## lapply to this list 
  lapply(yr.list,rfunc)
...
if(trash){
  ...
  rfunc <- function(yearso){
    ...
    ## make a a subset 
    bgsc_sub <- bgsc[which(bgsc$year == yearso),]
    print("after subset")
    print(dim(bgsc_sub))
    print(head(bgsc_sub))
    ...
    dcg <- ggplot() +
      geom_point(data = bgsc_sub,
    	      mapping = aes(x = pop.den.1000, y = bg_med_inc_real)) + 
      scale_x_continuous(labels = comma) +
      labs(title = "real median income over tiem dc",
           y = "real median income by block group")
  ...
  ## make a list of counties
  cnty.list <- unique(bgsc$area_type)
...
} # end of income distribution by msa and urban/sub/ex

##############################################
##### income graphs by population growth
#############################################

if(incg3){
  
  ### figure out rate of population growth by county, 1990 to 2018

  # shrink dataframe to relevant rows
  cnties$year.char <- paste0("pop.tot_",cnties$year)

  cnty.pop <- cnties[,c("year.char","county_name","FIPS","total_population")]
  
  
  # make it wide
  cnty.pop.wide <- spread(data = cnty.pop,
                          key = "year.char",
                          value = "total_population")
 
  # calculate change
  cnty.pop.wide$pop.change <- (cnty.pop.wide$pop.tot_2017 - cnty.pop.wide$pop.tot_1990)/cnty.pop.wide$pop.tot_1990
  
  # mark by growth quartile
  cnty.pop.wide$quartile <- as.integer(cut(cnty.pop.wide$pop.change, quantile(cnty.pop.wide$pop.change, probs=0:4/4), include.lowest=TRUE))
  table(cnty.pop.wide$quartile)

  #### add quartiles to block group data 
  
  
  #### function to make graphs by population growth quartile
  
  #### run function to make graphs by population growth quartile

  #### output list of jursidictions and population growth quartiles
  ### save csv for all counties and years
##### fix this 
  #cname <-paste0(janmtg,
  #               "income/msa/all_bgs_cnties_1990to2017_",
  #               dateo,
  #               ".csv")
  #write.csv(x = bgsc,
  #          file = cname,
