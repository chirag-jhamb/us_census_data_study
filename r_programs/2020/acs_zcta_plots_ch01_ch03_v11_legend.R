...
out_dir_county_maps <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2019/county_pages/")
...
data %>% dplyr::select(matches('^B25127_8_|B25127_15_|B25127_51_|B25127_58_')) %>% head(2)


data %>% dplyr::select(matches('^B25127_18_|B25127_25_|B25127_32_|B25127_39_|B25127_61_|B25127_68_|B25127_75_|B25127_82_')) %>% head(2)
...
#                           dplyr::select(ZCTA,total_single_family_households_since_2000,total_single_family_households_before_2000,
...
#data %>% dplyr::select(matches('*since_2000$')) %>% head(2)

data %>% dplyr::select(matches('*before_2000$')) %>% head(2)

data <- data %>% mutate('new_constructions'=dplyr::select(.,matches('*since_2000$')) 
                        %>% apply(1, sum, na.rm=TRUE))

data <- data %>% mutate('existing_constructions'=dplyr::select(.,matches('*before_2000$')) 
                        %>% apply(1, sum, na.rm=TRUE))
...
data <- data %>% mutate('total_single_family_households'=dplyr::select(.,matches('^total_single_family*')) 
                        %>% apply(1, sum, na.rm=TRUE))
...
plot_zcta_level_quantile_values <- function(df,colname,sfips,cfips){
 ...
 # if(first_lab_x==0 & colname_str=="share_single_family_new_constructions"){
  #  brks_cnt_df[ , "brks_name_x"] <- c(paste0("< 25th perc.","\n","no sf"),
    #                                   paste0("≥ 25th & < 50th perc.","\n",">0-",second_lab_x),
     #                                  paste0("≥ 50th & < 75th perc.","\n",second_lab_x+0.01,"-",third_lab_x),
      #                                 paste0("≥ 75th perc.","\n",third_lab_x+0.01,"-",fourth_lab_x))
    
  #}else{
    # brks_cnt_df[ , "brks_name_x"] <- c(paste0("< 25th perc.","\n","< ",change_scientific(first_lab_x+1)),
    #                                    paste0("≥ 25th & < 50th perc.","\n", first_lab_x+1,"-",second_lab_x),
    #                                    paste0("≥ 50th & < 75th perc.","\n",second_lab_x+1,"-",third_lab_x),
    #                                    paste0("≥ 75th perc.","\n", third_lab_x+1,"-",fourth_lab_x))
    
  brks_cnt_df[ , "brks_name_x"] <- c(paste0("< 25th perc.","\n","< ",change_scientific(first_lab_x+1)," units"),
                                     paste0("≥ 25th & < 50th perc.","\n", change_scientific(first_lab_x+1),"-",change_scientific(second_lab_x)," units"),
                                     paste0("≥ 50th & < 75th perc.","\n",change_scientific(second_lab_x+1),"-",change_scientific(third_lab_x)," units"),
                                     paste0("≥ 75th perc.","\n", change_scientific(third_lab_x+1),"-",change_scientific(fourth_lab_x)," units"))
  ...
  l2 <- l1 + 
    ...
    scale_y_continuous(labels = scales::comma, breaks = trans_breaks(identity, identity, n = n_break))+
  ...
  #ggbld <- ggplot_build(l1)
  #hline <- ggbld$layout$coord$labels(ggbld$layout$panel_params)[[1]]$y.major_source
  
  ggbld <- ggplot_build(l2)
  hline <- ggbld$layout$coord$labels(ggbld$layout$panel_params)[[1]]$x.major_source
  
  
  l3 <- l2 + 
    labs(y="Number of Zip Codes",x="")+
    theme(plot.title = element_text(size = 16, face = "bold"), 
          plot.subtitle = element_text(size = 15), 
          #axis.text.y = element_text(size = 20), 
          #axis.text.x = element_text(size = 20),
          axis.text = element_text(size = axis_text_size,color = axis_text_col),
          axis.title.x = element_text(size = axis_text_size,color = axis_text_col),
          axis.line = element_blank(),
          axis.ticks = element_blank(),
          axis.title.y = element_blank(), 
    ...
    geom_hline(yintercept = hline, color = "white") + 
  ...
  out_dir_c <- paste0(out_dir_county_maps,sfips,cfips,"/")
  
  if (colname_str=='new_constructions'){
    save_plot <- paste0(out_dir_c,"c1.l1.",sfips,cfips,"_",dateo,".jpg")
  } else if (colname_str=='new_existing_constructions_ratio'){
    save_plot <- paste0(out_dir_c,"c1.l2.",sfips,cfips,"_",dateo,".jpg")
  } else if (colname_str=='share_single_family_new_constructions'){
    save_plot <- paste0(out_dir_c,"c2.l1.",sfips,cfips,"_",dateo,".jpg")
  } 
  
  print(save_plot)
  
  ggsave(save_plot,plot = l3, dpi = 300, width = 17, height = 11, units = c("in"))
...
#ggplot(df, aes(x=df[["share_single_family_new_constructions"]]))+
...
plot_zcta_level_quantile_values <- function(df,colname,area_type){
  ...
  # put manual breaks as desired
  if(colname_str=="share_single_family_new_constructions"){
  #  df_not_1 <- df %>% filter(share_single_family_new_constructions!=1)
   # terciles <- quantile(df_not_1[[colname_str]],probs = seq(0, 1, 0.33))
    brks <- c(0,0.4,0.75,0.95,1)
    #brks <- c(0,0.42,0.86,0.99,1)
  }else{
  brks <- unique(quantile(df[[colname_str]],probs = seq(0, 1, 0.25)))
  }
  ...
  if(colname_str=="share_single_family_new_constructions"){  
    ...
    # brks_cnt_df[ , "brks_name_x"] <- c(paste0("< 33rd perc.","\n","< ",first_lab_x+0.01),
    #                                    paste0("≥ 33rd & < 66th perc.","\n",first_lab_x+0.01,"-",second_lab_x),
    #                                    paste0("≥ 66th & ≤ 99th perc.","\n",second_lab_x+0.01,"-",third_lab_x),
    #                                    paste0("= 100th perc.","\n",fourth_lab_x))
    
    brks_cnt_df[ , "brks_name_x"] <- c(paste0("< 40%"),
                                       paste0("≥ 40% to < 75%"),
                                       paste0("≥ 75% to < 95%"),
                                       paste0("≥ 95%"))
  ...
  #brks_cnt_df$brks_name_x <- gsub(" ", "\n", brks_cnt_df$brks_name_x)
  ...
  out_dir_c <- paste0(out_dir_county_maps,area_type,"/")

  out_dir_d <- paste0(out_dir_c,"data/")

  if (colname_str=='new_constructions' & area_type=="00000"){
    save_plot <- paste0(out_dir_c,"p1.l1.",area_type,"_",dateo,".jpg")
  }
