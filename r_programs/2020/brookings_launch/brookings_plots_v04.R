...
#
# copied from 
# /groups/brooksgrp/center_for_washington_area_studies/state_of_the_capitol_region/r_programs/2019/council_of_governments_presentation
# may 13, 2019 ## fixing up for brookings
# may 14, 2019
# may 16, 2019 ## more fixing up
# july 17, 2019 ## fixing label on one wrong graph
#
# brookings_plots_v04.R
#
##############################################################################
##############################################################################
...
out_dir_pres <- "/groups/brooksgrp/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2019/brookings_presentation/"
...
out_dir_ch01 <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2019/brookings_presentation/")
out_dir_ch02 <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2019/brookings_presentation/")
out_dir_ch03 <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2019/brookings_presentation/")
...
fairfax_fall_church_arrow <- total_suburban-round(total_farifax_falls/2) - 2000
...
p1 <- ggplot(df, aes(x = area_type, y = total_housing_units, fill =  group)) +
  ...
  geom_text(data=df1,aes(label = county_name_state_code), # << move each label down by 1 unit
            ...
            color = "white", size = 8) +
  annotate(geom="text", x=1.50, y=fairfax_fall_church_arrow + 10000, label="Fairfax &", 
  	   color="#525252", size=anno_text_size,
	   hjust = 1) +
  annotate(geom="text", x=1.50, y=fairfax_fall_church_arrow, label="Falls Church cities", 
  	   color="#525252", size=anno_text_size,
	   hjust = 1)

# save the plot
ggsave(paste0(out_dir_pres,"p1_g2_",dateo,".jpg"),
       plot = p1, dpi = 300, width = 20, height = 11, units = c("in"))


######## graph with just 3 bars by urban suburban and exurban ####################################
### calculate placement of arrows and text end here

df2 <- group_by(df, area_type)
df2 <- summarize(df2, thu = sum(total_housing_units))

use_colors <- c("#1f78b4","#a6cee3","#b2df8a")

df2

# plot the graph
p1 <- ggplot(df2, aes(x = area_type, y = thu)) +
...
# save the plot
ggsave(paste0(out_dir_pres,"p1_g2_3barsonly_",dateo,".jpg"),
       plot = p1, dpi = 300, width = 20, height = 11, units = c("in"))
...
p1 <- ggplot(data = acs_cnt_2000_2017_comp , aes(x = median_housing_value_2000, y = housing_units_pct_change_from_2000)) +
  ...
  stat_smooth(data=acs_cnt_2000_2017_comp_pct_2000_wo_Loudoun,method="lm_left", 
  	      fullrange=TRUE,col='#525252',se=FALSE) +
  geom_text_repel(aes(label=county_name, colour=factor(area_type)), 
  		  size=6,fontface = "bold", segment.size = 0)+
  scale_x_continuous(labels = comma,
  		     limits= c(0, 450000), 
		     breaks = c(seq(0,450000,100000)),
		     expand = c(0,5)) +
  labs(x = "median housing value, 2000", 
       y="% change, no. of hsng units, 2000-2017",
       color = "Parameter") + 
  scale_color_manual(values = c("Urban" = "#1f78b4","Suburban"="#a6cee3","Exurban"="#b2df8a"))+
  annotate(geom="text", x=330000, y=30, label="all counties", color="#bdbdbd", size=9)+
  annotate(geom="text", x=35000, y=51, label="without loudoun", color="#525252", size=9)+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        ...
        axis.title.x = element_text(margin = margin(t=20, r=20, b=0, l=20)),
        plot.title = element_text(size=25),
        legend.title = element_blank(),
        legend.position="none",
  ...
  annotate(geom="text", x=425000, y=80, label="Urban", color="#045a8d", size=10)+
  annotate(geom="text", x=425000, y=75, label="Suburban", color="#2b8cbe", size=10)+
  annotate(geom="text", x=425000, y=70, label="Exurban", color="#74c476", size=10)
...
ggsave(paste0(out_dir_pres,"p2_g1_",dateo,".jpg"),
       plot = p2, dpi = 300, width = 20, height = 11, units = c("in"))

################ plot with one period ######################

df 

p2 <- ggplot(df[which(df$Unit_Type == "Before 2000"),] ,  
             aes(x = Units_Count, y = unit_freq, fill = Unit_Type)) +
  geom_bar(position = "dodge", stat = "identity")+
  labs(x = "", y = "", colour = "Parameter")+
  scale_fill_manual("Subject", values=colours)+
  scale_shape_manual(values = c(16, 21)) +
...
# save the graph
ggsave(paste0(out_dir_pres,"p2_g1_before2000only_",dateo,".jpg"),
       plot = p2, dpi = 300, width = 20, height = 11, units = c("in"))
...
g1 <- ggplot(data = df, aes(x = county_name_state_code, y = single_family_prop, fill=group)) +
  ...
  theme(panel.grid.major = element_blank(),
        ...
        axis.text = element_text(size = axis_text_size),
        legend.position="none",
        plot.title = element_text(size = subt_size, hjust = 1))+
  scale_y_reverse() +
  coord_flip()
...
g2 <- ggplot(data = df, aes(x = county_name_state_code, y = total_new_units,fill=group)) +
  ...
  theme(panel.grid.major = element_blank(),
        ...
        axis.ticks.y = element_blank(),
        legend.position="none",
        plot.title = element_text(hjust = 0, size = subt_size))+
  coord_flip()
...
# set relative widths for graphs
grwid <- c(2.0/9,2.4/9,4.6/9)

## graph with all three panels
p1 <- grid.arrange(gg1,gg.mid,gg2,
		   ncol=3,
		   widths=grwid)

## graph with empty first panel
t <- textGrob("")
p01 <- grid.arrange(t,gg.mid,gg2,ncol=3,widths=grwid)

# save the graph
ggsave(paste0(out_dir_pres,"p2_g2_",dateo,".jpg"),
       plot = p1, dpi = 300, width = 22, height = 10, units = c("in"))

# save the graph w/o single family share panel
ggsave(paste0(out_dir_pres,"p2_g2_noshare_",dateo,".jpg"),
       plot = p01, dpi = 300, width = 22, height = 10, units = c("in"))
...
# plot the graph -- just the housing price
...
# save the graph
ggsave(paste0(out_dir_pres,"p3_g1_price_only_",dateo,".jpg"),
       plot = p1, dpi = 300, width = 20, height = 11, units = c("in"))
...
ggsave(paste0(out_dir_pres,"p3_g2_",dateo,".jpg"),
       plot = p1, dpi = 300, width = 22, height = 11, units = c("in"))
...
print("read map")
  ...
  cborders1960c <- cborders1960[which((cborders1960$COUNTY %in% c("001") & 
  		   	               cborders1960$STATE %in% c("11"))|
                                      (cborders1960$COUNTY %in% c("009","017","021","031","033") & 
				       cborders1960$STATE %in% c("24"))|
                                      (cborders1960$COUNTY %in% c("037") & 
				       cborders1960$STATE %in% c("54"))|
                                      (cborders1960$COUNTY %in% c("013","043","047","059","061","107","153","157","177","179","187", "510","600","610","630","683","685") & cborders1960$STATE %in% c("51"))),]

print("short map")

  cborders1960c <- cborders1960c %>% mutate("area_type"=
    ifelse((COUNTY %in% c("001") & STATE %in% c("11")) | 
           (COUNTY %in% c("013","510") & STATE %in% c("51")),"Urban",
      ifelse((COUNTY %in% c("033","031") & STATE %in% c("24"))|
             (COUNTY %in% c("059","600","610") & STATE %in% c("51")),"Suburban","Exurban")))
...
cborders1960_exurban <- cborders1960 %>% filter((COUNTY %in% c("009","017","021") & STATE=="24")|
                                                   (COUNTY %in% c("043","047","061","107","153","157","177","179","187","630","683","685") & STATE=="51")| (COUNTY=="037" & STATE=="54"))
...
#  geom_sf_label(data = cborders1960_urban, aes(label = NAME))+
 # geom_sf_label(data = cborders1960_suburban, aes(label = NAME))+
  ...
  annotate("text", x = -77.35, y = 38.9, label = "Fairfax County", size=siz_text)+
  ...
  annotate("text", x = -77.48, y = 39.01, label = "Falls Church City", size=siz_text)+
  ...
  annotate("text", x = -77.46, y = 38.67, label = "Fairfax City", size=siz_text)+
  #annotate("segment", x = -76.78, xend = -76.75, y = 39.03, yend = 39.14, colour = "black")+
  annotate("segment", x = -77.3, xend = -77.45, y = 38.85, yend = 38.68, colour = "grey",size=size_anno)+  ## Fairfax City
  annotate("segment", x = -77.18, xend = -77.45, y = 38.89, yend = 39, colour = "grey",size=size_anno)+    ## Fairfax City
  annotate("segment", x = -77.13, xend = -77.35, y = 38.88, yend = 38.61, colour = "grey",size=size_anno)+   ## Fairfax City
  annotate("segment", x = -77.07, xend = -77.12, y = 38.815, yend = 38.532, colour = "grey",size=size_anno)+  ## Fairfax City
  coord_sf(crs = st_crs(4326))+
  theme_map() 


  ggsave(paste0(out_dir_pres,"map_urban_",dateo,".jpg"), 
		plot = p1, dpi = 300, 
		width = 11, height = 11, 
		units = c("in"))
...
#  geom_sf_text(data = cborders1960_urban_new, aes(label = NEW_NAME))+
 ...
 # annotate("segment", x = -77.48388, xend = -76.75, y = 38.74797, yend = 39.25, colour = "black")+
  ...
  ggsave(paste0(out_dir_pres,"map_exurban_",dateo,".jpg"), 
		plot = p1, dpi = 300, 
		width = 11, height = 11, 
		units = c("in"))
