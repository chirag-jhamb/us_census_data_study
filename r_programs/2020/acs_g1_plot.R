...
out_dir_intro <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2020/2020_report/introduction/")

# out_dir_counties <- paste0(groupDir,"/center_for_washington_area_studies/state_of_the_capitol_region/r_output/2019/county_pages/")
...
cborders1960_urban_new <- st_as_sf(cborders1960_urban_new)
cborders1960_suburban_new <- st_as_sf(cborders1960_suburban_new)
p1 <- ggplot() +
...
# p1
save_path <- paste0(out_dir_intro,dateo,"_county_borders_washington_urban_area.jpg")
print("Saving:")
print(save_path)
...
cborders1960_exurban_new <- st_as_sf(cborders1960_exurban_new)
# draw the plot with county names in it without any demographic data
p1 <- ggplot() +
  geom_sf(data = cborders1960_urban_new, fill="#1f78b4", size=size_border,color=border_col)+
  geom_sf(data = cborders1960_suburban_new, fill="#a6cee3",size=size_border,color=border_col)+
...
# p1
save_path <-paste0(out_dir_intro,dateo,"_county_borders_washington_metro_area.jpg")
ggsave(save_path, plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))
...
# #p1
#
# #ggsave(paste0(out_dir_intro,"i1.m2_",dateo,"_county_borders_washington_urban_area_no_names.jpg"), plot = p1, dpi = 300, width = 16, height = 11, units = c("in"))
#
#
# ###################### Draw each county ##################################
#
...
#   geom_sf(data = df_county, fill=col_fill, size=1)+
#   coord_sf(crs = st_crs(4326))+
#   theme_map() +
#   theme(legend.position = "bottom",legend.background = element_rect(color = NA))
#
#
# out_dir_c <- paste0(out_dir_counties,sfips,cfips,"/")
#
# ggsave(paste0(out_dir_c,"c1.m0.",sfips,cfips,"_",dateo,".jpg"),
