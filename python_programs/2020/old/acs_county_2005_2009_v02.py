...
docFilePath = r'/groups/brooksgrp/census/american_community_survey/2005_2009_5year_estimates/raw_data/' \
              r'Sequence_Number_and_Table_Number_Lookup.xls'
...
def get_data(table_title, table_id, state):
    ...
    data_path = r'/groups/brooksgrp/census/american_community_survey/2005_2009_5year_estimates/raw_data/' \
...
def get_summary_data(state):
    ...
    AgeOfHousing_df.iloc[:,12:] = AgeOfHousing_df.iloc[:,12:].apply(pd.to_numeric).\
                                        div(AgeOfHousing_df["Total:"].apply(pd.to_numeric),axis=0)
...
filename = datetime.datetime.today().strftime('%Y%m%d')+'_cnty_acs_'+estimate_start_year+'_'+estimate_end_year+'.csv'
print(filename)
print(type(filename))
