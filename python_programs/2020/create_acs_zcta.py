...
# Chirag Jhamb------>Email:chirag@email.gwu.edu
# %%%%%%%%%%%%% Date:
# V2 November - 22 - 2018
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# %%-----------------------------------------------------------------------
...
def get_summary_year_data(start_year, end_year):
    ...
    outDir = r"/groups/brooksgrp/center_for_washington_area_studies/state_of_the_capitol_region/python_output/2020/zcta_data"
...
#################### A. get table data #########################
    ...
    HouseholdFamily_df = get_data('HOUSEHOLDS BY PRESENCE OF PEOPLE UNDER 18 YEARS BY HOUSEHOLD TYPE','B11005')
    # Income_df = get_data('HOUSEHOLD INCOME IN THE PAST 12 MONTHS (IN ' + end_year + ' INFLATION-ADJUSTED DOLLARS)', 'B19001')

    data_frames = [Population_df, Age_df, Race_df, Median_Income_df, HouseholdFamily_df] #,Income_df]
